...
# Create a single class called Account. Upon initialization, it should receive an owner (str) and
# a starting amount (int, optional, 0 by default). It should also have an attribute called _transactions (empty list).
# Create the following methods:
#     • handle_transaction(transaction_amount)
#         ◦ If the balance becomes less than zero, raise ValueError with
#     the message "sorry cannot go in debt!" and break the transaction.
...
#     • When you print a representational string of an account instance, the output should be
# in the format "Account({owner}, {amount})".
